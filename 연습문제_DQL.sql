...
-- 3. EMPLOYEES 테이블에서 FIRST_NAME이 'Steven', LAST_NAME이 'King'인 사원을 조회하시오. (어려움, 해답확인요망)
SELECT EMPLOYEE_ID AS 사원번호
     , FIRST_NAME 
     , LAST_NAME
  FROM EMPLOYEES
 WHERE FIRST_NAME LIKE CONCAT('Steven', '%') OR LAST_NAME LIKE CONCAT('King', '%');
...
SELECT EMPLOYEE_ID
     , JOB_ID
  FROM EMPLOYEES
 WHERE JOB_ID LIKE CONCAT('IT_PROG', '%') OR JOB_ID LIKE CONCAT('ST_MAN', '%');
...
SELECT EMPLOYEE_ID 
     ...
     , SALARY 
     , COMMISSION_PCT 
     , SALARY * COMMISSION_PCT AS 실제커미션
  FROM EMPLOYEES
 WHERE COMMISSION_PCT IS NOT NULL;      --복습해
