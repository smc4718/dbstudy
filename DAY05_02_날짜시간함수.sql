-- 1. 현재 날짜 및 시간   (외우지 말고 이해만 하고 정리만 해둬라)
...
-- 세션타임존 기준 시간
SELECT SESSIONTIMEZONE    -- 우리나라의 타임존
     , CURRENT_DATE       -- DATE 형식
     , CURRENT_TIMESTAMP  -- TIMESTAMP 형식
  FROM DUAL;
  
-- 2. 날짜를 원하는 형식으로 조회하기
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD')  -- TO_DATE 는 원하는 형식으로 조회하는 게 아니라 이 형식으로 날짜를 해석하라는 방식.
     , TO_CHAR(SYSDATE, 'YYYY-MM-DD AM HH:MI:SS')
     , TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS')
     , TO_CHAR(SYSTIMESTAMP, 'YYYY-MM-DD HH24:MI:SS.FF3') -- 밀리초(천분의 1초) 포함.  ' FF? ' 몇자리까지 보여줄까 : FF3 - 3자리까지 보여줘.
  FROM DUAL;
...
SELECT TO_CHAR(SYSDATE + 1,         'YYYY-MM-DD AM HH:MI:SS')         --1일 후
     ...
     , TO_CHAR(SYSDATE + 1/2460/60, 'YYYY-MM-DD AM HH:MI:SS') --1초 후
  FROM DUAL;
  
SELECT SYSDATE - TO_DATE('23/07/01', 'YY/MM/DD')
     , TRUNC(SYSDATE - TO_DATE('23/07/01', 'YY/MM/DD'))  --경과한 일수를 정확하게 계산할 수 있다. (예시 : 비밀번호 변경 안내창)
  FROM DUAL;
...
SELECT EXTRACT(YEAR FROM SYSDATE)           -- 년
     ...
     , EXTRACT(HOUR FROM SYSTIMESTAMP)+9    -- 시, SYSDATE 는 기본값 년,월,일이기 때문에 시간부터는 SYSTIMESTAMP 를 써야 한다. / Asia/Seoul 기준
     , EXTRACT(MINUTE FROM SYSTIMESTAMP)    -- 분, EXTRACT를 쓰면 UTC(표준시) 기준으로 출력된다. (우리나라는 UTC + 9시간)
     , EXTRACT(SECOND FROM SYSTIMESTAMP)    -- 초
     , TRUNC(EXTRACT(SECOND FROM SYSTIMESTAMP))
     , TO_CHAR(SYSDATE, 'YYYY') -- TO_CHAR 함수를 추출용도로 사용하는 것도 가능.
  FROM DUAL;
  
-- 6. 요일을 기준으로 특정 날짜 구하기
SELECT NEXT_DAY(SYSDATE, '수')
     , NEXT_DAY(SYSDATE, '목')
     , NEXT_DAY(SYSDATE-8, '수')   --당일 처리 원리 때문에 -7일이 아니라 -8일 해줘야 함.
  FROM DUAL;
