...
-- 활용1. WITH문과 재귀 쿼리 (어려움, 그냥 참고용)  --자기가 자기를 호출해서 계속 사용하는 것 : 재귀 쿼리.
WITH
...
-- 활용2. WITH문과 재귀 쿼리를 활용한 사원의 레벨 표시하기 (짤 줄 몰라도 괜찮음, 난이도 극상. 공부X)
-- MANAGER가 몇 명인가에 따른 LVL 표시하기
-- MANAGER가 0명이다 : LVL = 1
-- MANAGER가 1명이다 : LVL = 2
WITH MY_SUBQUERY(LVL, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID) AS (   --마이서브쿼리가 LEVEL 이라는 칼럼을 가지고 있는 것)
