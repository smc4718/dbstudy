/*
    ...
    2. M:N 관계를 가진 2개의 테이블은 직접 관계를 맺는 것이 불가능하다. (개념적으로만 가능, 실제 구현은 불가능)
...
-- 학생과 과목은 관계가 없기 때문에 위 아래 아무데나 만들어도 된다. 수강신청 테이블 위에만 만들면 됨.
...
-- *수강신청 테이블 만들기 전에 미리 과목테이블이 만들어져 있어야 한다. (부모 자식 관계)
-- 다대다관계는 연습 많이.
-- 테이블을 먼저 만들기 -> 칼럼만들고 밑에 제약조건으로 만들기.
--REFERENCES <- 단어 주의.
