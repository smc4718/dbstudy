...
CREATE TABLE BOOK_T (
  ...
  , BOOK_NAME  VARCHAR2(100 BYTE)      NULL  --> VARCHAR2 를 '가변 길이 문자' 라고 함.
...
CREATE SEQUENCE BOOK_SEQ
    START WITH 1                --전체옵션 적는 건 신물이 날 정도로 연습해라, 옵션을 작성해야 되는 순서는 없다. 쓰고 싶은 것 먼저 써라.
    INCREMENT BY 1
    NOMAXVALUE  --최댓값없음
    NOMINVALUE  --최솟값없음
    NOCYCLE
    CACHE 20    --번호표 20개를 미리 뽑아 놓는다.
    ORDER;       --뽑은 순서대로 사용하겠다.
...
/*
    ...
    1004     박세리   대한민국  NULL           --NULL값은 문자열이 아니어서 '' 로 묶으면 안됨, 빈칸이아니라 NULL 이라고 적어줘야함.
*/
...
CREATE SEQUENCE CUST_SEQ
    ...
    NOCYCLE
    CACHE 20
    ORDER;      --ORDER를 안 적으면 NOORDER 가 되어 순서대로 안 되고, 미리 뽑아 놓은 번호표도 날라간다.
--전체옵션중 안 적어도 기본값으로 자동사용되는 것들은 (INCREMENT BY 1, NOMAXVALUE, NOMINVALUE, NOCYCLE, CACHE 20) 안 써도 된다.
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES(CUST_SEQ.NEXTVAL, '박지성', '영궁', '000-000-000');
...
CREATE SEQUENCE ORDER_SEQ       --시퀀스 이름은 자유롭게 써도 된다. 이렇게 쓴 건 강사님이 그냥 임의로 지정하신것.
    START WITH 1
    INCREMENT BY 1
    NOMAXVALUE     --사용하고 싶으면 MAXVALUE 99999 뒤에 숫자 적으면 됨.
    NOMINVALUE
    NOCYCLE
    CACHE 20
    ORDER;
...
ALTER TABLE BOOK_T  ADD CONSTRAINT FK_BOOK_ORDER     FOREIGN KEY(BOOK_ID) REFERENCES BOOK_T(BOOK_ID)     ON DELETE SET NULL;
...
SELECT BOOK_NAME   --혹은 '*' 
 ...
 --           ★★ LIKE '%' || '올림픽' || '%';   <- 이렇게 연습하면 더 좋음.

-- 8. 가격이 가장 비싼 책을 조회하시오.
SELECT *
  FROM BOOK_T
 WHERE PRICE = (SELECT MAX(PRICE)              --> 서브쿼리와 메인쿼리는 조회대상이 동일해야함.
                  FROM BOOK_T);
...
-- 두 번째 풀이법 ↓ 
-- SELECT RANK() OVER(ORDER BY PRICE DESC) AS 순위, BOOK_ID, BOOK_NAME, PUBLISHER, PRICE
--   FROM BOOK_T;
...
SELECT O.ORDER_ID AS 주문번호
 ...
 -- ORDER_DT에 TO_DATE 필요 없는 이유는 INSERT에서 TO_DATE 처리를 해서 값을 집어넣었기 때문에 WHERE 절에서 TO_DATE 를 쓸 필요 없다.
 -- 비트윈 날짜 비교할 때 TO_DATE로 만들자.
...
SELECT O.ORDER_ID AS 구매번호
  ...
  FROM BOOK_T B INNER JOIN ORDER_T O
    ...
    ON C.CUST_ID = O.CUST_ID
 WHERE O.ORDER_DT NOT BETWEEN TO_DATE('20/07/07') AND TO_DATE('20/07/07');
 
 
-- 12. 가장 최근에 구매한 고객의 이름, 책이름, 주문일자를 조회하시오. (가장 최근 = MAX)
-- 고객명  책이름            주문일자
-- 장미란  역도 단계별 기술  20/07/10
SELECT C.CUST_NAME   AS 고객명
     , B.BOOK_NAME AS 책이름
     , O.ORDER_DT  AS 주문일자
  FROM BOOK_T B INNER JOIN ORDER_T O        --중복될 수 없는 것부터 조인 앞에 나와주면 좋다. 성능상 좋음.
    ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
    ON C.CUST_ID = O.CUST_ID
...
-- 15. '김연아'가 구매한 도서수를 조회하시오.           --COUNT를 쓰려면 GROUP BY가 필요하다. (GROUP BY를 안 쓰면 전체갯수밖에 안 나온다.)
-- 고객명  구매도서수
-- 김연아  2
SELECT C.CUST_NAME AS 고객명
     , COUNT(O.ORDER_ID) AS 구매도서수    --구매한 도서수이지만, 책의 갯수가 아니라 주문횟수를 세는 것이다.
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;  --그룹바이에 CUST_NAME만 있으면 이름이 '김연아'인 모든 사람이 한 사람으로 겹쳐진다.
                                   --그래서 CUST_ID (고객번호)를 반드시 적어주어서 정확하게 조회되게 한다.
--정확한 명칭은 구매도서수 가 아니라 구매한횟수이다.
...
-- 출판사     판매된책수
-- 굿스포츠   4
-- 삼성당     0     --안 팔린 책도 나온 걸 보니 외부조인이다.
-- 나이스북   3
-- 대한미디어 1
-- 이상미디어 2
SELECT B.PUBLISHER     AS 출판사
     , COUNT(ORDER_ID) AS 판매된책수
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID
...
-- 18. 모든 구매 고객의 이름과 총구매액(PRICE * AMOUNT)을 조회하시오. 구매 이력이 있는 고객만 조회하시오. (구매이력없으면 조회X 내부조인)
-- 고객명  총구매액
-- 박지성  116000
-- 김연아  19000
-- 장미란  62000
...
SELECT C.CUST_NAME AS 고객명
  ...
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON B.BOOK_ID = O.BOOK_ID
 GROUP BY C.CUST_ID, C.CUST_NAME;    --> 항상 CUST_ID , CUST_NAME 은 같이 가야 정확한 대상을 조회가능. 
...
-- 20. 총구매액이 2~3위인 고객의 이름와 총구매액을 조회하시오. (목록 구하는 쿼리문은 이 정도 난이도로 나옴)
-- 고객명  총구매액
-- 추신수  86000
-- 장미란  62000
SELECT A.CUST_NAME AS 고객명
     , A.TOTAL AS 총구매액
  FROM (SELECT RANK() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS RK     --인라인 뷰 (정렬을 한 후 값을 내야 하기 때문에 먼저 실행하도록 FROM문에 쓴 것)
             , C.CUST_NAME
             , SUM(B.PRICE * O.AMOUNT) AS TOTAL
          FROM CUSTOMER_T C INNER JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
            ON B.BOOK_ID = O.BOOK_ID
         GROUP BY C.CUST_ID, C.CUST_NAME) A   --> 인라인뷰의 이름을 'A' 라고 지정해준 것.
 WHERE A.RK BETWEEN 2 AND 3;
 -->서브쿼리 인라인뷰의 이름을 'A' 라고 지정하고 메인쿼리에 칼럼에 A가상테이블 지정.
 --인라인 뷰 (정렬을 한 후 값을 내야 하기 때문에 먼저 실행하도록 FROM문에 쓴 것)